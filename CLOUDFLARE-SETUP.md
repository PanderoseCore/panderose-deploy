...
2. **AI Crawlers tab** — review the crawler list once traffic starts
   flowing. For each verified AI crawler (`ClaudeBot`, `GPTBot`,
   `OAI-SearchBot`, `PerplexityBot`, `Google-Extended`, etc.), leave
   **Allow** — that's the whole point, we want them reading the site. Block
   only ones that show up as pure scrapers with no citation/answer product
   behind them, if you decide you don't want a particular one.
