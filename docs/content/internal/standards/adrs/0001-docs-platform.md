...
FastAPI service's own repo (see `docs/specs/README.md`) — never
hand-written, so it can't drift from what's actually deployed.

## Consequences

- Node/npm build step is now part of this repo's deploy pipeline (it
  wasn't before — the marketing site has none). Handled by
